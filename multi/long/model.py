...
class ElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        sent_logits = self.qa_outputs(sent_gru_output)
        ...
        sent_start_vectors = self.start_projection_layer(sent_gru_output)
        sent_end_vectors = self.end_projection_layer(sent_gru_output)
        batch_size = sent_start_vectors.size(0)
        sent_len = sent_start_vectors.size(1)

        expanded_start_vectors = sent_start_vectors.unsqueeze(2).expand(-1, -1, sent_len, -1)
        expanded_end_vectors = sent_end_vectors.unsqueeze(1).expand(-1, sent_len, -1, -1)
        ...
        triu_mask = _make_triu_mask(sent_len).unsqueeze(0).expand(batch_size, -1, -1).cuda()
        ...
        if start_positions is not None and end_positions is not None:
            ...
            row_label = torch.zeros([batch_size, sent_len], dtype=torch.long).cuda()
            col_label = torch.zeros([batch_size, sent_len], dtype = torch.long).cuda()
            valid_label = torch.zeros([batch_size, sent_len, sent_len], dtype=torch.long).cuda()
            ...
            row_loss = matrix_loss_fct(matrix_logits.view(-1, sent_len),
                row_label.view(-1)).reshape(batch_size, sent_len)
            col_loss = matrix_loss_fct(matrix_logits.transpose(1, 2).reshape(batch_size * sent_len, sent_len),
                col_label.view(-1)).reshape(batch_size, sent_len)
            ...
            start_span_loss = span_loss_fct(sent_start_logits, start_position)
            end_span_loss = span_loss_fct(sent_end_logits, end_position)
        ...
        return sent_start_logits, sent_end_logits, matrix_logits, valid_prob
